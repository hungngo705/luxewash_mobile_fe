/**
 * Top-Up Screen
 * POST /api/v1/wallets/top-up -> redirects to PayOS checkoutUrl
...
export default function TopUpScreen() {
  ...
  const getReturnUrl = (): string => {
    if (Platform.OS === 'web') {
      return `${window.location.origin}/wallet/top-up`;
    }
    return 'luxewashmobilefe://wallet/top-up';
  };

  const getCancelUrl = (): string => {
    if (Platform.OS === 'web') {
      return `${window.location.origin}/wallet/top-up?status=cancelled`;
    }
    return 'luxewashmobilefe://wallet/top-up?status=cancelled';
  };
  ...
  const handleTopUp = async () => {
    ...
    try {
      ...
      if (res.statusCode === 200 && res.data?.checkoutUrl) {
        ...
        if (Platform.OS === 'web') {
          ...
          if (canOpen) {
            ...
            alert('Không thể mở liên kết thanh toán');
            setIsLoading(false);
            return;
          }
        }
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.section}>
          ...
          {numericAmount > 0 && numericAmount < 10000 && (
            <Text style={styles.minWarning}>Số tiền nạp tối thiểu: 10.000đ</Text>
          )}
        </View>
      ...
      {!polling && paymentStatus !== 'success' && (
        <View style={styles.footer}>
          <TouchableOpacity
            style={[styles.submitBtn, numericAmount < 10000 && styles.submitBtnDisabled]}
            onPress={handleTopUp}
          ...
          >
            {isLoading ? (
              ...
              <Text style={styles.submitBtnText}>
                Thanh toán {numericAmount > 0 ? formatCurrency(numericAmount) : ''}
              </Text>
            )}
          </TouchableOpacity>
        </View>
