...
export function BookingProvider({ children }: { children: ReactNode }) {
  ...
  const setVehicleService = useCallback(
    (licensePlate: string, serviceId: number) => {
      setState((prev) => {
  ...
  const createBooking = useCallback(async (): Promise<{
    ...
    for (const vehicle of state.selectedVehicles) {
      if (!state.selectedServices.has(vehicle.licensePlate)) {
        return {
          success: false,
          error: `Vui lòng chọn dịch vụ cho xe ${vehicle.licensePlate}`,
        };
      }
    }
    ...
    try {
      ...
      if (response.statusCode === 200 || response.statusCode === 201) {
        ...
        return {
          success: false,
          error: response.message || "Tạo đặt lịch thất bại",
        };
      }
    } catch (error: unknown) {
    ...
    async (
      ...
      try {
        ...
        if (response.statusCode === 200 || response.statusCode === 201) {
          ...
          return {
            success: false,
            error: response.message || "Hủy đặt lịch thất bại",
          };
        }
      } catch (error: unknown) {
